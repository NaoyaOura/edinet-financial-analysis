...
import jp.ac.example.xbrl.db.JQuantsDailyPricesDao;
...
/**
 * jquants-fetch-prices コマンド。
 *
 * edinet_jquants_mapping に登録済みの銘柄の日次株価を取得して jquants_daily_prices に保存する。
 */
public class JQuantsFetchPricesCommand {

    private final AppConfig config;
    private final DatabaseManager dbManager;

    public JQuantsFetchPricesCommand(AppConfig config, DatabaseManager dbManager) {
        this.config = config;
    ...
    public void execute(String[] args) throws Exception {
        ...
        if (fiscalYear == 0) {
            throw new IllegalArgumentException("--year オプションは必須です。例: jquants-fetch-prices --year 2023");
        }
        ...
        // 年度 → 日付範囲（日本の年度: 4月始まり）
        String from = fiscalYear + "-04-01";
        String to = (fiscalYear + 1) + "-03-31";
        ...
        System.out.println("株価取得対象: " + mappings.size() + " 銘柄 / 期間: " + from + " 〜 " + to);
        ...
        for (EdinetJQuantsMappingDao.MappingRecord mapping : mappings) {
            ...
            try {
                List<JQuantsApiClient.DailyQuoteRecord> quotes = apiClient.fetchDailyQuotes(jquantsCode, from, to);
                try (Connection conn = dbManager.getConnection()) {
                    JQuantsDailyPricesDao dao = new JQuantsDailyPricesDao(conn);
                    for (JQuantsApiClient.DailyQuoteRecord q : quotes) {
                        dao.upsert(new JQuantsDailyPricesDao.DailyPriceRecord(
                            q.code(), q.date(), q.open(), q.high(), q.low(), q.close(),
                            q.volume(), q.adjustmentClose(), q.adjustmentVolume()
                        ));
                    }
                }
                successCount++;
                ...
                System.err.println("株価取得失敗 [" + jquantsCode + "]: " + e.getMessage());
                errorCount++;
            }
        }

        System.out.println("株価取得完了: 成功=" + successCount + " 件、失敗=" + errorCount + " 件");
    }
